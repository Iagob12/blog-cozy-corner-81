"""
OpenRouter Gemini Client - Usa Gemini 2.0 Flash via OpenRouter
Limite muito maior que API direta do Google
"""
import json
import re
...
class OpenRouterGeminiClient:
    """
    Cliente para usar Gemini 3 Flash via OpenRouter
    ...
    def __init__(self, api_key: str):
        ...
        self.model = "google/gemini-3-flash-preview"  # Gemini 3 Flash!
        
        logger.info(f"[{datetime.now()}] [OPENROUTER] Cliente inicializado com Gemini 3 Flash")
    ...
    async def executar_prompt(
        ...
        """
        Executa prompt usando OpenRouter
        ...
        logger.info(
            f"[{datetime.now()}] [OPENROUTER] Executando '{task_type}' "
        ...
        logger.info(f"[{datetime.now()}] [OPENROUTER] Resposta recebida com sucesso")
    ...
    async def executar_prompt_raw(
        ...
        logger.info(
            f"[{datetime.now()}] [OPENROUTER] Executando RAW '{task_type}' "
            f"({len(prompt)} chars)"
        )
    ...
    async def testar_conexao(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[{datetime.now()}] [OPENROUTER] ✗ Teste falhou: {e}")
            return False


# Singleton instance
_openrouter_client: Optional[OpenRouterGeminiClient] = None


def get_openrouter_gemini_client(api_key: str = None) -> OpenRouterGeminiClient:
    """Retorna instância singleton do OpenRouterGeminiClient"""
    ...
    if _openrouter_client is None:
        ...
        _openrouter_client = OpenRouterGeminiClient(api_key)
