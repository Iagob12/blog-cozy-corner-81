"""
Groq Client - Cliente para usar Groq (Llama 3.3 70B)
Extremamente rápido e com free tier generoso!
"""
import json
import re
...
class GroqClient:
    """
    Cliente para usar Groq
    
    Vantagens:
    - Extremamente rápido (mais rápido que Gemini!)
    - Free tier: 30 requisições/minuto
    - Llama 3.3 70B (excelente qualidade)
    ...
    def __init__(self, api_key: str):
        ...
        logger.info(f"[{datetime.now()}] [GROQ] Cliente inicializado com Llama 3.3 70B")
    ...
    async def executar_prompt(
        ...
        logger.info(
            f"[{datetime.now()}] [GROQ] Executando '{task_type}' "
        ...
        logger.info(f"[{datetime.now()}] [GROQ] Resposta recebida com sucesso")
    ...
    async def executar_prompt_raw(
        ...
        logger.info(
            f"[{datetime.now()}] [GROQ] Executando RAW '{task_type}' "
            f"({len(prompt)} chars)"
        )
        ...
        logger.info(f"[{datetime.now()}] [GROQ] Resposta RAW recebida ({len(response_text)} chars)")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.info(f"[{datetime.now()}] [GROQ] Aguardando {wait_time}s antes de retry...")
                    await asyncio.sleep(wait_time)
        
        # Se todas as tentativas falharam
        logger.error(f"[{datetime.now()}] [GROQ] Todas as tentativas falharam")
        raise last_error
    ...
    def _parsear_json(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[{datetime.now()}] [GROQ] Erro ao parsear JSON: {e}")
            logger.error(f"[{datetime.now()}] [GROQ] Texto recebido: {texto[:500]}...")
    ...
    async def testar_conexao(self) -> bool:
        ...
        try:
            logger.info(f"[{datetime.now()}] [GROQ] Testando conexão...")
            
            response = await self._executar_com_retry("Responda apenas: OK")
            
            if "OK" in response.upper():
                logger.info(f"[{datetime.now()}] [GROQ] ✓ Conexão OK")
                return True
            else:
                return False
        
        except Exception as e:
            logger.error(f"[{datetime.now()}] [GROQ] ✗ Teste falhou: {e}")
            return False
...
def get_groq_client(api_key: str = None) -> GroqClient:
    """Retorna instância singleton do GroqClient"""
    global _groq_client
